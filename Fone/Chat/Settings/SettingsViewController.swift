...
class SettingsViewController: UITableViewController, SettingsTableViewCellDelegate, UserProfileImageNameSettingDelegate, NotificationDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let showPreview = UserDefaults.standard.object(forKey: SBConstants.ID_SHOW_PREVIEWS) as? Bool {
            self.showPreview = showPreview
        }
        
        if let createDistinctChannel = UserDefaults.standard.object(forKey: SBConstants.ID_CREATE_DISTINCT_CHANNEL) as? Bool {
    ...
    func didChangeSwitchButton(isOn: Bool, identifier: String) {
      if identifier == SBConstants.ID_SHOW_PREVIEWS {
            UserDefaults.standard.set(isOn, forKey: SBConstants.ID_SHOW_PREVIEWS)
            UserDefaults.standard.synchronize()
            self.showPreview = isOn
        }
        else if identifier == SBConstants.ID_CREATE_DISTINCT_CHANNEL {
            UserDefaults.standard.set(isOn, forKey: SBConstants.ID_CREATE_DISTINCT_CHANNEL)
            UserDefaults.standard.synchronize()
