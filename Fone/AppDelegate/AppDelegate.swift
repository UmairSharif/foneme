...
class AppDelegate: UIResponder, UIApplicationDelegate, BranchDelegate, CLLocationManagerDelegate, GroupChannelsUpdateListDelegate {
    ...
    let viewController = UIStoryboard(name: "Home", bundle: nil).instantiateViewController(withIdentifier: "VideoCallVC") as! VideoCallVC
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        SocialLoginManager.shared.application(
            application,
...
//        })
        ...
        UIBarButtonItem.appearance().tintColor = hexStringToUIColor(hex: "333333")
        return true
    }
    
    //MARK:- GET USER PROFILE
    func getUserProfile(cnic foneId: String = "",_ completion : @escaping (_ model : UserDetailModel? , _ success : Bool) -> Void )
    {
            if let loginToken = UserDefaults.standard.object(forKey: "AccessToken") as? String, loginToken.isEmpty == false {
                
                let params = ["Cnic" : foneId] as [String:Any]
                print("params: \(params)")
    ...
    func application(
        ...
        if !branchHandled {
            return SocialLoginManager.shared.application(
                app,
                open: url,
                options: options
            )
    ...
    @objc func canHandleNotification() {
    ...
    func configureApplicationSetup() {
        // Social login initializer
        SocialLoginManager.shared.initialize()
