        ...
        //{
            if logArray.count > indexPath.row {
                ...
                if log.status == "Out Going" {
                    ...
                    cell.callStatusImage.image = UIImage(named: "outgoing_call_ic")
                    cell.callStatusLbl.textColor = hexStringToUIColor(hex: "3E79ED")
                } else if log.status == "InComing" {
                    cell.callStatusLbl.text = "Incoming Call"
                    cell.callStatusImage.image = UIImage(named: "ic_incoming_call")
                    cell.callStatusLbl.textColor = hexStringToUIColor(hex: "229753")
                } else {
                    cell.callStatusLbl.text = "Missed Call"
                    cell.callStatusImage.image = UIImage(named: "missed_call_ic")
                    cell.callStatusLbl.textColor = hexStringToUIColor(hex: "FE1B37")
                }
                ...
                if Calendar.current.isDateInToday(date ?? Date()) {
                    ...
                    cell.timeLbl.text = "Today, " + dateString
                    
                } else if Calendar.current.isDateInYesterday(date ?? Date()) {
                    dateFormatter.dateFormat = "hh:mm a"
                    dateFormatter.timeZone = TimeZone.current
                    let dateString = dateFormatter.string(from: date ?? Date())
                    cell.timeLbl.text = "Yesterday, " + dateString
                } else {
